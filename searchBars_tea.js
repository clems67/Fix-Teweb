...
loopSearchBars = setInterval(() => {
  ...
  if (filter !== oldFilterBU) {
    Filter("ctl00_cph_tea_gvwNewLine_ctl02_ddlCodeBU", filter);
    oldFilterBU = filter;
  }
  filter = document.getElementById("inputProject").value;
  if (filter !== oldFilterProject) {
    Filter("ctl00_cph_tea_gvwNewLine_ctl02_ddlProjet", filter);
...
function InitiateSearchBars() {
  ...
  let activityNode = document.getElementById("ctl00_cph_tea_gvwNewLine");
...
function RemoveMarginTop() {
  document.getElementById("ctl00_cph_tea_gvwNewLine").style = "margin-top:0";
}
